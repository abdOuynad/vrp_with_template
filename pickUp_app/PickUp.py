...
Original file is located at
    https://colab.research.google.com/drive/16TrN5KcQC9hiNWIcqQHDUZOcdN67VxEE
"""
...
class vrp_by_geopy:
    #
    def __init__(self, clients, client_data, cars, car_data, coordonne, init,
    ...
    #
    @client_data.setter
    def client_data(self, val):
        self._client_data = val
    #
    ...
    def add_client_to_df_dist_sorted(self, client, cord):
        ...
        #
        print('the distance in =>',len(new_client_column))
        print('coordonne ==>',self._coordonne)
        print('table ==>',new_client_column)
    ...
    #
    def add_clien_to_vrp_dict_with_template(self,client,cord):
        #
        print('welcom in the add client to vrp dict with default pos 0 ')
        pos_index = 0
        m = []
        #
        dict_dis = self.add_client_to_df_dist_sorted(client, cord)
        ...
        #
        print('client sorted ==>', sorted_list)
        self._vrp_affec_sol[index[2]]['sol'] = sorted_list
        #
        # return index[2],self._vrp_affec_sol
        return index[2]
        #
    #
    def add_clien_to_vrp_dict(self,client, cord, pos):
        #
        print('wellcom in the add client to vrp dict')
        print('pos ==>',pos)
        #
        pos_index =0
        m = []
    ...
    #
    def add_client_visualisation(self):
        colors = ['red', 'blue', 'green', 'purple', 'orange', 'darkred', 'lightred', 'beige', 'darkblue', 'darkgreen',
        'cadetblue', 'darkpurple', 'white', 'pink', 'lightblue', 'lightgreen', 'black', 'lightgray']
        d ={}
        indx = 0
        #
        map = folium.Map(location=[36.789,2.348998],
                        zoom_control=3,control_scale=True)
        #
        print('sol ==>',self._vrp_affec_sol)
        for x,y in self._vrp_affec_sol.items():
            lines = []
            #
            print('sol x ==>', x)
            print('sol y ==>', y)
            #print('sol ==>',list(x.values()['pos'])[0])
            for car in self._car_data:
                if x == car[0]:
                    lines.append(car[1])
                    folium.Marker(location=car[1],
                                  popup=car[0],
                                  tooltip= 'Driver',
                                  icon= folium.Icon(color=colors[indx],icon='driver')
                                  ).add_to(map)
                    break
            #
            for client in y['sol']:
                #
                for c in self._client_data:
                    if client == c[0]:
                        lines.append(c[1])
                        folium.Marker(location=c[1],
                                      popup=client,
                                      tooltip='Client',
                                      icon= folium.Icon(color=colors[indx])
                                      ).add_to(map)
                        break
                #
            #
            indx+=1
            print('lines ==>',lines)
            folium.PolyLine(lines,tooltip= x).add_to(map)
        #
        map.get_root().height ='300px'
        d['iframe'] = map.get_root()._repr_html_()
        d['sol'] = self._vrp_affec_sol
        #
        return d
    #
    def add_clien_to_vrp_dict_new(self,pos,client, cord):
        ...
        for x,y in self._vrp_affec_sol.items():
            ...
            #
        index = min(m)
        print('i dnt know what i mean at index ===>',index)
        print('vrp solution ===>',self._vrp_affec_sol[index[2]])
        clients = self._vrp_affec_sol[index[2]]['sol']
    ...
    #
    #
    def add_clien_to_vrp_dict_pro(self, client, cord, pos, status):
        #
        print('wellcom in the add client to vrp dict')
        print('pos ==>', pos)
        #
        ...
        for x, y in self._vrp_affec_sol.items():
            ...
            #
            y['pos'] = pos[pos_index]
            y['status'] = status[pos_index]
            k = y['pos']
            #
            if (y['status'] == 'progress'):
                #
                init = y['sol'][k]
                list_client = y['sol'][k:]
                new_dict_dis = [x for x in dict_dis if x[1] in list_client]
    ...
    def ajout(self, clients, client, k):
        ...
        print('new liste client ==>',new_list_client)
        #
        d = {}
        #
    ...
    #
    def cars_sorted (self,cars):
      #
      resultat = []
      for car in cars:
        sol = {}
        car_sorted = sorted(list(car.values())[0][len(cars):], key=lambda x: (x[0], x[1]))
        print(car_sorted)
        sol[list(car.keys())[0]]=car_sorted
        resultat.append(sol)
      #
      return resultat
    #
    def vrp_init_cars (self,cars):
      result = []
      basket = []
      index = 0
      #
      while(len(result)!=len(cars)):
        #
        for car in cars:
          #
          sol = {}
          #
          if (list(car.keys())[0] not in [x[2] for x in basket]):
            #begin if for the car its exixst in the basket or not
            #print(car)
            #print(basket)
            #
            if ( list(car.values())[0][index][1] in [x[1] for x in basket]):
              #
              #print('match')
              #
              for c in basket:
                #
                if(c[1] == list(car.values())[0][index][1]):
                  #
                  #print('basket',c)
                  #
                  if (c[0]>=list(car.values())[0][index][0]):
                    #
                    #print('basket inf',c[0],'<',list(car.values())[0][index][0])
                    basket.remove(c)
                    #
                    for v in result:
                      #
                      if(list(v.keys())[0] == c[2]):
                        #
                        result.remove(v)
                        #
                      #
                    #
                    #sol[list(car.keys())[0]] = list(car.values())[0][index]
                    sol[list(car.keys())[0]] = [list(car.values())[0][index][1]]
                    #
                    result.append(sol)
                    #
                    basket.append((list(car.values())[0][index][0],list(car.values())[0][index][1],list(car.keys())[0]))
                    #
                  #
                #
              #
    ...
    def remove_car(self,cars):
      #
      for car in cars:
        self._clients.remove(car)
      #
    #
    def drop_car(self,cars):
      #
      for car in cars:
        #
        self.drop_client_to_df(car)
        #
      #
    #
    def find_and_annulation_dict_with_template(self,client):
        #
        index = 0
        #
        for x,y in self._vrp_affec_sol.items():
            if client in y['sol']:
                ...
                y['pos'] = 0
                list_client = y['sol']
                new_list_affec = self.annulation(client, list_client,0)
    ...
    def affect(slef, init, list_clients_rest, dict_client):
        #print('affect ===>',dict_client)
        #print('list rest ===>',list_clients_rest)
    ...
    def affectation_client(self, init_vrp, dict_client):
        ...
        while (len(rest) != 0):
            ...
            for x in init_vrp_copy:
                ...
                if len(rest) == 0:
                    break
        #print("result copy ==>", init_vrp_copy)
        self.vrp_sol_to_dict(init_vrp_copy)

    #
    def affectation_client_with_data(self, init_vrp, dict_client,client_data,car_data):
        #
        print('init vrp ==>',init_vrp)
        ...
        #
        resultat = self.vrp_sol_to_dict_with_data(init_vrp_copy,client_data,car_data)
        return resultat
    #
    ...
    #
    def vrp_sol_to_dict_with_data(self,vrp_sol,clients_data,cars_data):
        groups = ['groupe_1','groupe_2','groupe_3','groupe_4','groupe_5','groupe_6',
                  'groupe_7','groupe_8','groupe_9','groupe_10','groupe_11','groupe_12']
        colors = ['red', 'blue', 'green', 'purple', 'orange', 'darkred', 'lightred', 'beige', 'darkblue', 'darkgreen',
        'cadetblue', 'darkpurple', 'white', 'pink', 'lightblue', 'lightgreen', 'black', 'lightgray']
        d = {}
        d_copy = {}
        ...
        for indx,x in enumerate(vrp_sol):
            s = {}
            s_copy = {}
            clients = []
            lines = []
            #
            s_copy['sol']=list(x.values())[0]
            #
            s_copy['pos']=0
            #
            s_copy['status']='progress'
            #
            d_copy[list(x.keys())[0]]=s_copy
            #
            for car in cars_data:
                if list (x.keys())[0] == car[0]:
                    #
                    folium.Marker(
                        location= car[1],
                        popup= car[0],
                    ...
                    s['coordonne'] = car[1]
                    break
            for client in list(x.values())[0]:
                n ={}
                for client_data in clients_data:
                    if client == client_data[0]:
                        #
                        folium.Marker(
                            location= client_data[1],
                            tooltip= 'client',
                            popup= client_data[0],
                            icon=folium.Icon(color =colors[indx])
                        ).add_to(map)
                        #
                        lines.append(client_data[1])
                        #
                        n['name'] = client
                        n['coordonne'] = client_data[1]
                        clients.append(n)
                        break
            s['sol'] =clients
            d[list(x.keys())[0]] = s
            #
            folium.PolyLine(lines,tooltip=list(x.keys())[0]).add_to(map)
            #
        map.get_root().height ='300px'
        d['iframe'] = map.get_root()._repr_html_()
        self._vrp_affec_sol = d_copy
        return d
    #
...
if __name__ == '__main__':
    ...
    #
    drivers=[{'v1':(15.38956,12.36587)}, {'v2':(14.65898,13.32587)}, {'v3':(13.65898,12.32587)}, {'v4':(15.65898,13.52587)}]
    #
    depo = (36.7149768, 3.2094611)
    #cars = ['v1', 'v2', 'v3', 'v4']
    #
    cars = [list(x.keys())[0] for x in drivers]
    ci = [list(x.values())[0] for x in drivers]
    #
    #
    crdn = ci + crdn
    #
    list_client = cars + list_client
    #
    ...
    print('convert list client ==>',tour._clients)
    tour.drop_car(cars)
    ...
    print('ajout resultat ',tour._vrp_affec_sol)
    print('ajoute ==>',tour.add_clien_to_vrp_dict_pro('abdouyyy', (12.05, 15.05), [2, 3, 2, 3],['stop','progress','stop','progress']), '\n')
